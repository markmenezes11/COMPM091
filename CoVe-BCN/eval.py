...
parser.add_argument("--type", type=str, default="CoVe", help="What sentence embeddings to use (GloVe, CoVe_without_GloVe, InferSent or CoVe). For CoVe, [GloVe(w)CoVe(w)] embeddings will be used. For CoVe_without_GloVe, GloVe(w) will not be included.")
...
from sentence_encoders import GloVeEncoder, CoVeEncoder, GloVeCoVeEncoder, InferSentEncoder
...
elif args.transfer_task == "TREC6":
    dataset = TREC6Dataset(args.datadir, encoder, dry_run=(args.mode == 2))
elif args.transfer_task == "TREC50":
    dataset = TREC50Dataset(args.datadir, encoder, dry_run=(args.mode == 2))
elif args.transfer_task == "TREC6_lower":
    dataset = TREC6LowerDataset(args.datadir, encoder, dry_run=(args.mode == 2))
elif args.transfer_task == "TREC50_lower":
    dataset = TREC50LowerDataset(args.datadir, encoder, dry_run=(args.mode == 2))
else:
