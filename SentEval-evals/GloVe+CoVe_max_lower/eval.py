...
parser = argparse.ArgumentParser(description='SentEval Evaluation of GLoVe+CoVe Sentence Representations in the form [GLoVe(w)+CoVe(w) for each word w in sentence]')
...
def batcher(params, batch):
    ...
    for raw_sentence in batch:
        ...
        if len(sentence) == 0:
            embedding = np.zeros((1, 900), dtype=float)
        ...
        # Iinitialise embedding to that of first word
        embedding = []
        for num in vector_list[0]:
            embedding.append(num)
        for num in context_vectors[0][0]:
            embedding.append(num)
