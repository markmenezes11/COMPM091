...
class PyTorchClassifier(object):
    ...
    def trainepoch(self, X, y, epoch_size=1):
        ...
        for i in range(0, len(X)):
            ...
            X_embeddings.append(np.array([files_in_mem[filename][index]]))
        X_embeddings = np.vstack(X_embeddings)
        X_embeddings = self.cast_to_float_tensor(X_embeddings)
        files_in_mem.clear()
        gc.collect()
        ###############################

        for _ in range(self.nepoch, self.nepoch + epoch_size):
            permutation = np.random.permutation(len(X_embeddings))
            all_costs = []
            for i in range(0, len(X_embeddings), self.batch_size):
                # forward
                idx = torch.LongTensor(permutation[i:i + self.batch_size])
                if isinstance(X_embeddings, torch.cuda.FloatTensor):
                    idx = idx.cuda()
                Xbatch = Variable(X_embeddings.index_select(0, idx))
    ...
    def score(self, devX, devy):
        ...
        for i in range(0, len(devX)):
            ...
            if filename not in files_in_mem:
                with open(filename) as f:
                    files_in_mem[filename] = pickle.load(f)
            devX_embeddings.append(np.array([files_in_mem[filename][index]]))
        ...
        for i in range(0, len(devX_embeddings), self.batch_size):
            Xbatch = Variable(devX_embeddings[i:i + self.batch_size], volatile=True)
        ...
        accuracy = 1.0*correct / len(devX_embeddings)
        return accuracy
