...
def batcher(params, batch):
    ...
    for raw_sentence in batch:
        ...
        vector_list = []
        if len(sentence) == 0:
            embeddings.append(np.array([[np.repeat(0.0, 100)]])[0]) # TODO: Do something more intuitive here if sentence is empty?
            continue
        ...
        embeddings.append(np.array([params.cove(vector_tensor, length_tensor).cpu().data.numpy()[0][0][:100]]))
